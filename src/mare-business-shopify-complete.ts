...
function nodes(value: unknown): JsonObject[] {
  ...
  return Array.isArray(connection.nodes)
    ? connection.nodes.filter((item) => item && typeof item === "object").map((item) => item as JsonObject)
    : [];
}
...
function mapVariant(raw: VariantNode): JsonObject {
  const inventoryItem = object(raw.inventoryItem);
  const unitCost = object(inventoryItem.unitCost);
  const inventoryLevels = nodes(inventoryItem.inventoryLevels).map((level) => {
    const location = object(level.location);
    return {
      id: level.id ?? null,
  ...
  return {
    ...
    selected_options: Array.isArray(raw.selectedOptions) ? raw.selectedOptions : [],
...
function mapProduct(raw: ProductNode, completeVariants: VariantNode[]): JsonObject {
  const media = nodes(raw.media).map((item, index) => {
    const image = object(item.image);
    const original = object(item.originalSource);
    return {
      id: item.id ?? null,
      position: index + 1,
  ...
  const collections = nodes(raw.collections).map((collection) => ({
    id: collection.id ?? null,
    handle: collection.handle ?? null,
    title: collection.title ?? null,
  }));
  const seo = object(raw.seo);
  const variants = completeVariants.map(mapVariant);
  return {
    id: raw.id ?? null,
    title: raw.title ?? null,
...
async function loadAllVariants(product: ProductNode, env: MareBusinessShopifyEnv): Promise<VariantNode[]> {
  const initial = (product.variants?.nodes || []) as VariantNode[];
  const result = [...initial];
  let hasNextPage = Boolean(product.variants?.pageInfo?.hasNextPage);
  let after = normalize(product.variants?.pageInfo?.endCursor) || null;
...
export async function readShopifyCatalogComplete(args: JsonObject, env: MareBusinessShopifyEnv): Promise<JsonObject> {
  ...
  const includeCsv = bool(args.include_csv, true);
  let after: string | null = null;
  let hasNextPage = true;
  const products: JsonObject[] = [];
  let paginatedVariantProducts = 0;
  ...
  while (hasNextPage && products.length < maxProducts) {
    ...
    for (const product of connection.nodes || []) {
      ...
      products.push(mapProduct(product, completeVariants));
    }
    hasNextPage = Boolean(connection.pageInfo?.hasNextPage);
    after = normalize(connection.pageInfo?.endCursor) || null;
  ...
  const jsonArtifact = await storeBusinessArtifact(env, {
    kind: "shopify_catalog_complete_json",
    filename: `shopify-catalog-complete-${generatedAt.slice(0, 10)}.json`,
    ...
    metadata: {
      ...
      complete_variant_pagination: true,
      paginated_variant_products: paginatedVariantProducts,
    },
  });
  const csvArtifact = includeCsv ? await storeBusinessArtifact(env, {
    kind: "shopify_catalog_complete_csv",
    filename: `shopify-catalog-complete-${generatedAt.slice(0, 10)}.csv`,
    mime_type: "text/csv",
    encoding: "utf-8",
  ...
  const variantCount = products.reduce((sum, product) => sum + Number(product.variant_count || 0), 0);
  const mediaCount = products.reduce((sum, product) => sum + Number(product.media_count || 0), 0);
  return {
    ok: true,
