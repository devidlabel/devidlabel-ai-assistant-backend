...
const PATH = "/internal/ops/mc2back40-cohort-2026-08-08";
const GITHUB_REPOSITORY = "devidlabel/devidlabel-ai-assistant-backend";
const START = "2025-08-10T00:00:00+02:00";
const END = "2026-08-09T00:00:00+02:00";
...
function isMc2Vendor(value: string | null | undefined): boolean {
  ...
  return v.includes("mc2") || v.includes("saint barth") || v.includes("st barth") || v.includes("saintbarth");
}
...
export async function handleMc2Back40CohortProbe(request: Request, env: Mc2Back40Env): Promise<Response | null> {
  const url = new URL(request.url);
  if (url.pathname !== PATH) return null;
  if (request.method !== "GET") return json({ ok: false, operation: "mc2back40_cohort", reason: "method_not_allowed" }, 405);
  if (!(await isGitHubRepositoryWriteToken(bearer(request)))) return json({ ok: false, operation: "mc2back40_cohort", reason: "not_found" }, 404);

  const query = `processed_at:>=${START} processed_at:<${END}`;
  ...
  const rows: Array<{ email_sha256: string; latest_mc2_order: string; latest_any_order: string; mc2_orders: number; dormant_30d: boolean }> = [];
  ...
  rows.sort((a, b) => b.latest_mc2_order.localeCompare(a.latest_mc2_order));

  return json({
    ok: true,
    operation: "mc2back40_cohort",
    data_policy: "sha256_email_only_no_pii",
    timeframe: { start: START, end_exclusive: END, dormant_cutoff: DORMANT_CUTOFF },
    orders_scanned: scanned,
    pages_scanned: pages,
    line_item_truncation_warnings: truncatedLineItems,
    unique_direct_customers_in_window: state.size,
    mc2_customers: rows.length,
    mc2_dormant_30d: rows.filter((r) => r.dormant_30d).length,
    customers: rows,
  });
}
