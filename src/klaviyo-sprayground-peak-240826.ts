...
const MICRO_NAME = "DL | 2026-08-25-26 | Sprayground | High Intent | Hot Window";
const MICRO_SUBJECT = "Lo Sprayground che stavi guardando è ancora qui";
const MICRO_PREVIEW = "Se ci stai tornando sopra, controlla ora disponibilità e modelli più richiesti.";
const MICRO_HOLD = "2026-08-26T16:30:00+00:00";
...
function filterLiteral(value:string):string{return value.replace(/\\/g,"\\\\").replace(/'/g,"\\'");}
...
async function senderIdentity(apiKey:string):Promise<{from_email:string;from_label:string;reply_to_email:string}>{const source=await must(apiKey,`/api/campaigns/${CAMPAIGN_27}?include=campaign-messages`);const row=obj(source.data);const mid=campaignMessageId(row);if(!mid)throw new Error("source_sender_message_missing");const message=await must(apiKey,`/api/campaign-messages/${mid}`);const definition=obj(obj(obj(message.data).attributes).definition);const content=obj(definition.content);const fromEmail=normalize(content.from_email);const fromLabel=normalize(content.from_label);const reply=normalize(content.reply_to_email)||fromEmail;if(!fromEmail||!fromLabel||!reply)throw new Error("source_sender_identity_missing");return{from_email:fromEmail,from_label:fromLabel,reply_to_email:reply};}
async function findCampaignByName(apiKey:string,name:string):Promise<JsonObject|null>{const q=new URLSearchParams();q.set("filter",`and(equals(messages.channel,'email'),contains(name,'${filterLiteral(name)}'))`);q.set("include","campaign-messages");q.set("page[size]","10");const payload=await must(apiKey,"/api/campaigns?"+q.toString());for(const item of Array.isArray(payload.data)?payload.data:[]){const row=obj(item);if(normalize(obj(row.attributes).name)===name)return row;}return null;}
async function createOrReuseMicroCampaign(apiKey:string,audienceId:string,excludedId:string):Promise<JsonObject>{const existing=await findCampaignByName(apiKey,MICRO_NAME);if(existing){const status=normalize(obj(existing.attributes).status);if(status!=="Draft")throw new Error(`micro_campaign_exists_not_draft:${status}`);let mid=campaignMessageId(existing);if(!mid){const full=await must(apiKey,`/api/campaigns/${normalize(existing.id)}?include=campaign-messages`);mid=campaignMessageId(obj(full.data));}return{ok:true,status:"already_exists",external_write_performed:false,campaign_id:normalize(existing.id),campaign_message_id:mid||null,campaign_status:status,campaign_name:MICRO_NAME,audience_id:audienceId,excluded_audience_id:excludedId};}const sender=await senderIdentity(apiKey);const payload=await must(apiKey,"/api/campaigns",{method:"POST",body:JSON.stringify({data:{type:"campaign",attributes:{name:MICRO_NAME,audiences:{included:[audienceId],excluded:excludedId?[excludedId]:[]},send_strategy:{method:"static",datetime:MICRO_HOLD,options:{is_local:false}},send_options:{use_smart_sending:true},tracking_options:{add_tracking_params:true,custom_tracking_params:[],is_tracking_clicks:true,is_tracking_opens:true},"campaign-messages":{data:[{type:"campaign-message",attributes:{definition:{channel:"email",label:MICRO_NAME,content:{subject:MICRO_SUBJECT,preview_text:MICRO_PREVIEW,from_email:sender.from_email,from_label:sender.from_label,reply_to_email:sender.reply_to_email}}}}]}}}})});const row=obj(payload.data);const id=normalize(row.id);if(!id)throw new Error("micro_campaign_created_without_id");let mid=campaignMessageId(row);if(!mid){await sleep(1200);const full=await must(apiKey,`/api/campaigns/${id}?include=campaign-messages`);mid=campaignMessageId(obj(full.data));}if(!mid)throw new Error("micro_campaign_message_missing");return{ok:true,status:"draft_created",external_write_performed:true,campaign_id:id,campaign_message_id:mid,campaign_status:normalize(obj(row.attributes).status)||"Draft",campaign_name:MICRO_NAME,audience_id:audienceId,excluded_audience_id:excludedId};}
...
async function execute(env:Env):Promise<JsonObject>{
  ...
  const recentBuyerId=normalize(obj(segments.recent_buyer_exclusion).id);

  const microCreated=await createOrReuseMicroCampaign(apiKey,HIGH_INTENT,recentBuyerId); const microId=normalize(microCreated.campaign_id); if(!microId)throw new Error("micro_wave_campaign_id_missing");
  const microUpdate=await updateKlaviyoCampaignDraft({approval_confirmation:"UPDATE KLAVIYO DRAFT",campaign_id:microId,subject:MICRO_SUBJECT,preview_text:MICRO_PREVIEW,html_body:emailHtml("intent",products),use_smart_sending:true},env);
