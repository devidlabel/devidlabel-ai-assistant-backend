...
async function findSegment(apiKey:string,name:string):Promise<JsonObject|null>{ const q=new URLSearchParams(); q.set("filter",`equals(name,'${filterLiteral(name)}')`); q.set("additional-fields[segment]","profile_count"); q.set("page[size]","10"); const p=await must(apiKey,"/api/segments?"+q.toString()); for(const item of Array.isArray(p.data)?p.data:[]){ const row=obj(item); if(normalize(obj(row.attributes).name)===name) return row; } return null; }
