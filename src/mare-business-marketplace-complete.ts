import { getBusinessArtifact, storeBusinessArtifact, type MareBusinessShopifyEnv } from "./mare-business-shopify.js";
import { readShopifyCatalogComplete } from "./mare-business-shopify-complete.js";

type JsonObject = Record<string, unknown>;
type Channel = "google_merchant" | "meta_catalog" | "tiktok_catalog" | "amazon_json_listings" | "spartoo_csv" | "miinto_csv" | "generic_csv";

const ALLOWED_CHANNELS: readonly Channel[] = [
  "google_merchant", "meta_catalog", "tiktok_catalog", "amazon_json_listings", "spartoo_csv", "miinto_csv", "generic_csv",
];
...
function selectedOption(variant: JsonObject, pattern: RegExp): string {
  const option = array(variant.selected_options).find((item) => pattern.test(normalize(item.name)));
...
function variantRows(products: JsonObject[], includeOutOfStock: boolean): Array<{ product: JsonObject; variant: JsonObject }> {
  const result: Array<{ product: JsonObject; variant: JsonObject }> = [];
...
function canonicalRow(product: JsonObject, variant: JsonObject, args: JsonObject): JsonObject {
  ...
  const currentPrice = numeric(variant.price);
  const compareAtPrice = numeric(variant.compare_at_price);
  const discounted = compareAtPrice > currentPrice && currentPrice > 0;
  const regularPrice = discounted ? compareAtPrice : currentPrice;
  ...
  return {
    ...
    condition: "new",
    price: `${regularPrice.toFixed(2)} ${currency}`,
    sale_price: discounted ? `${currentPrice.toFixed(2)} ${currency}` : "",
    current_price: `${currentPrice.toFixed(2)} ${currency}`,
    compare_at_price: compareAtPrice > 0 ? `${compareAtPrice.toFixed(2)} ${currency}` : "",
    ...
    quantity: stock,
    size: selectedOption(variant, /size|taglia/i),
    color: selectedOption(variant, /color|colore/i),
...
async function loadCompleteProducts(args: JsonObject, env: MareBusinessShopifyEnv): Promise<JsonObject[]> {
  ...
  const artifactId = normalize(object(object(catalog.artifacts).json).artifact_id);
  if (!artifactId) throw new Error("complete_catalog_artifact_missing");
...
function buildChannelContent(channel: Channel, rows: JsonObject[], args: JsonObject): { content: string; mimeType: string; filename: string; warnings: string[] } {
  ...
  if (channel === "amazon_json_listings") {
    ...
    const messages = rows.map((row, index) => ({
      ...
      attributes: {
        ...
        externally_assigned_product_identifier: normalize(row.gtin)
          ? [{ type: "ean", value: row.gtin, marketplace_id: marketplaceId || "REQUIRES_MARKETPLACE_ID" }]
          : [],
        purchasable_offer: [{
          currency: normalize(args.currency) || "EUR",
          our_price: [{ schedule: [{ value_with_tax: numeric(String(row.current_price).split(" ")[0]) }] }],
          marketplace_id: marketplaceId || "REQUIRES_MARKETPLACE_ID",
        }],
    ...
    return {
      content: JSON.stringify({
        header: { sellerId: sellerId || "REQUIRES_SELLER_ID", version: "2.0", issueLocale: "it_IT" },
        messages,
      }, null, 2),
  ...
  const headers = [
    "id", "item_group_id", "title", "brand", "gtin", "price", "sale_price", "quantity", "size", "color", "link", "image_link", "product_type",
  ];
  return {
    content: csvFromRows(rows, headers),
    mimeType: "text/csv",
...
export async function generateMarketplaceFeedComplete(args: JsonObject, env: MareBusinessShopifyEnv): Promise<JsonObject> {
  ...
  const products = await loadCompleteProducts(args, env);
  const rows = variantRows(products, bool(args.include_out_of_stock, false)).map(({ product, variant }) => canonicalRow(product, variant, args));
  const built = buildChannelContent(channel, rows, args);
  const artifact = await storeBusinessArtifact(env, {
    kind: `marketplace_feed_complete_${channel}`,
    ...
    metadata: {
      ...
      canonical_source: "shopify_admin_graphql_complete",
...
export async function generateMatrixifyCatalogComplete(args: JsonObject, env: MareBusinessShopifyEnv): Promise<JsonObject> {
  const products = await loadCompleteProducts(args, env);
  ...
  for (const product of products) {
    ...
    for (let index = 0; index < rows.length; index += 1) {
      ...
      const record: JsonObject = {
        ...
        "Title": normalize(transformations.title) || product.title,
        ...
        "SEO Title": normalize(transformations.seo_title) || product.seo_title,
        "SEO Description": normalize(transformations.seo_description) || product.seo_description,
        "Image Src": image.url,
  ...
  const artifact = await storeBusinessArtifact(env, {
    kind: "matrixify_catalog_complete_csv",
    filename: `matrixify-catalog-complete-${new Date().toISOString().slice(0, 10)}.csv`,
    mime_type: "text/csv",
    encoding: "utf-8",
    content: `${lines.join("\n")}\n`,
    metadata: {
