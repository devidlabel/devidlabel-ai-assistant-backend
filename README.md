...
Variabili supportate dal Worker per Shopify Dev Dashboard client credentials auth:
...
- `SHOPIFY_CLIENT_ID`: Client ID della app in Shopify Dev Dashboard → app → Impostazioni → Credenziali. Deve essere configurato come secret Cloudflare.
- `SHOPIFY_CLIENT_SECRET`: Client Secret della app in Shopify Dev Dashboard → app → Impostazioni → Credenziali. Deve essere configurato come secret Cloudflare.
- `SHOPIFY_ADMIN_ACCESS_TOKEN`: token Admin API statico legacy opzionale. Se presente ha priorità sul client credentials flow; deve essere sempre un secret Cloudflare.
...
Secrets consigliati Cloudflare per il nuovo flusso Shopify Dev Dashboard:
...
Compatibilità legacy opzionale, solo se il token statico esiste già:

```bash
npx wrangler secret put SHOPIFY_ADMIN_ACCESS_TOKEN
```

Non usare il “Token di automazione dell’app” come sostituto del token Admin API generato server-side. Il backend genera l'access token Admin API chiamando `POST https://${SHOPIFY_SHOP_DOMAIN}/admin/oauth/access_token` con `grant_type: "client_credentials"`, `SHOPIFY_CLIENT_ID` e `SHOPIFY_CLIENT_SECRET`. Il token non viene mai restituito al frontend, non viene mai inviato al browser, non viene loggato e non va mai committato.

`SHOPIFY_API_VERSION` e `SHOPIFY_RECOMMENDATION_CACHE_TTL_SECONDS` possono stare in `wrangler.toml`, dashboard Cloudflare o variabili ambiente. Non inserire token, Client Secret, API key OpenAI o altri secret in `wrangler.toml`, `.env.example`, README o tema Shopify.
...
La cache V1 è in-memory globale per isolate Cloudflare Worker. La chiave include intent, vendor, query corretta e gender/categoria quando presente. Il TTL è configurabile con `SHOPIFY_RECOMMENDATION_CACHE_TTL_SECONDS` e di default vale 3600 secondi. Anche il token ottenuto via client credentials usa una cache in-memory globale separata: se Shopify restituisce `expires_in`, il Worker lo riusa fino a `expires_in - 5 minuti`; se `expires_in` manca, usa un TTL prudente di circa 23 ore. Su cache hit `/chat` riusa lo snapshot; su cache miss calcola prodotti, vendite e alternative e salva lo snapshot. Se Shopify non è configurato o fallisce, la risposta resta valida e usa il fallback/static advice esistente.
...
2. Imposta i secret Cloudflare consigliati con `npx wrangler secret put SHOPIFY_CLIENT_ID`, `npx wrangler secret put SHOPIFY_CLIENT_SECRET` e `npx wrangler secret put SHOPIFY_SHOP_DOMAIN`. Usa `SHOPIFY_ADMIN_ACCESS_TOKEN` solo come compatibilità legacy opzionale se già disponibile.
